...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('🔧 Starting auto-schedule maintenance job...')
    ...
    for (const system of solarSystems) {
      ...
      const maintenance = await prisma.maintenanceRecord.create({
        data: {
          ...
          description: `Mantenimiento preventivo automático programado cada ${config.intervalDays} días`,
          autoGenerated: true,
      ...
      await prisma.maintenanceStatusHistory.create({
        data: {
          ...
          status: 'SCHEDULED',
          notes: 'Mantenimiento generado automáticamente por el sistema',
      ...
      console.log(
        `✓ Scheduled maintenance for ${system.systemName} on ${nextScheduledDate.toISOString()}`
      )
    }
